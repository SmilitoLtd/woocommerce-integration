...
namespace TestBlock\SmilitoLtd;
...
class Updater
{
	...
	private bool $active;
	private object|null $githubResponse = null;
	...
	private function getRepositoryInfo(): object
	{
	...
	{
		if (!property_exists($transient, 'plugins')) {
			return $transient;
		}
		...
		if ($checked = $transient->checked) {
			...
			$outOfDate = version_compare($latestVersion, $checked[$this->basename], 'gt');

			if ($outOfDate) {
				$newFiles = $info->zipball_url;
				$slug = current(explode('/', $this->basename));

				$plugin = new \stdClass();
				$plugin->url = $this->plugin['PluginURI'];
				$plugin->slug = $slug;
				$plugin->package = $newFiles;
